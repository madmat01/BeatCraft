...
export interface UseDrumSequencerReturn extends DrumSequencerState {
  ...
  playSound: (soundType: DrumSoundType) => void;
...
export const useDrumSequencer = (
  ...
  const [steps, setSteps] = useState<boolean[][]>(() => {
    ...
    return Array(drumSounds).fill(null).map(() => Array(totalSteps).fill(false));
  });
  ...
  useEffect(() => {
    ...
    // Notes for each drum sound
    const notes = ['C2', 'D2', 'F2', 'G2'];
    
    // Dispose of the previous sequence if it exists
    ...
    sequenceRef.current = new Tone.Sequence(
      (time, step) => {
        ...
        // Play the sounds for this step
        steps.forEach((row, soundIndex) => {
          if (row[step] && samplerRef.current) {
            try {
              samplerRef.current.triggerAttack(notes[soundIndex], time);
            } catch (error) {
              console.warn(`Error playing sound ${notes[soundIndex]}:`, error);
            }
          }
        });
      },
  ...
  const playSound = useCallback((soundType: DrumSoundType) => {
    ...
    try {
      // Play the sound
      samplerRef.current.triggerAttackRelease(noteMap[soundType], '16n');
    } catch (error) {
  ...
  const setPattern = useCallback((pattern: boolean[][]) => {
    ...
    const totalSteps = stepsPerBeat * totalBeats;
    const validatedPattern = pattern.map(row => {
      // If row is too short, pad with false
